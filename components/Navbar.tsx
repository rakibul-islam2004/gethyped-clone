  ...
  return (
    ...
    >
      <div className="flex justify-between items-center w-full max-w-[96em] h-[5.6em] lg:h-[6em] mx-auto px-[1em] md:px-[1.2em] lg:px-[1.6em]">
        ...
        <Link href="/" aria-label="Home link" className="relative z-[1000] h-[3em] lg:h-[3.8em] inline-block">
        ...
        <div 
          className={`
            ${isMobileMenuOpen 
              ? 'fixed inset-0 w-full h-[100svh] flex flex-col justify-center items-center bg-transparent z-[998] transition-opacity duration-300 opacity-100' 
              : 'hidden lg:flex absolute left-1/2 -translate-x-1/2 bg-brand-white rounded-[0.8em] h-[2.8em] p-[0.4em] gap-[0.4em] z-[998] overflow-hidden'}
          `}
        >
          {/* Menu Links */}
          <div className={`${isMobileMenuOpen ? 'flex flex-col gap-5 text-center text-[2em] font-semibold items-center z-10 transition-transform duration-500 delay-100 translate-x-0' : 'flex flex-row gap-[0.4em] h-full z-10 relative'}`}>
            {navLinks.map((link) => (
              <Link 
                key={link.title} 
                href={link.href} 
        ...
        {/* navbar_menu-bottom: Mobile CTA Wrapper */}
        <div className={`absolute inset-x-0 bottom-0 pb-[2.4em] flex justify-center items-center z-10 ${isMobileMenuOpen ? 'flex' : 'hidden'}`}>
          <div className="show-tablet">
            <Link href="#results" className="button-default is-nav-mobile w-inline-block" onClick={() => setIsMobileMenuOpen(false)}>
              <div className="button-default__inner">
                <span className="button-default__background is-nav-mobile"></span>
                <span className="button-default__text font-medium text-brand-white">Get Results</span>
        ...
        {/* navbar_menu-bg */}
        <div className={`absolute lg:hidden inset-y-0 right-0 w-full h-[100svh] bg-brand-pink z-[997] transition-transform duration-500 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-[100%]'}`}></div>
        
        {/* navbar_menu-bg-back backdrop */}
        <div className={`absolute lg:hidden inset-0 w-full h-[100svh] bg-brand-black/20 z-[996] transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}></div>

        {/* navbar_menu-trigger */}
        <div 
          className="lg:hidden relative z-[1000] bg-brand-pink cursor-pointer rounded-[0.6em] w-[2.8em] h-[2.8em] flex justify-center items-center flex-none"
          onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
        >
            <div className={`absolute w-[16px] h-[1.6px] bg-brand-black transition-transform duration-300 ${isMobileMenuOpen ? 'rotate-45 translate-y-0' : '-translate-y-[3.2px]'}`}></div>
            <div className={`absolute w-[16px] h-[1.6px] bg-brand-black transition-transform duration-300 ${isMobileMenuOpen ? '-rotate-45 translate-y-0' : 'translate-y-[3.2px]'}`}></div>
        </div>
