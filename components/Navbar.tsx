...
export function Navbar() {
  ...
  return (
    <>
    {/* === NAV BAR (logo, desktop menu, hamburger) === */}
    <nav
    ...
    >
      <div className="flex justify-between items-center w-full max-w-[96em] h-[5.6em] lg:h-[6em] mx-auto px-[1em] md:px-[1.2em] lg:px-[1.6em]">
          ...
          <div className="flex flex-row gap-[0.4em] h-full z-10 relative">
            {navLinks.map((link) => (
              <Link
                ...
                className="button-color-swoosh is-menu w-inline-block"
              >
        ...
        {/* Desktop CTA */}
        <div className={`hide-tablet-mobile hidden lg:block z-[1000] ${isMobileMenuOpen ? 'lg:block hidden' : ''}`}>
        ...
        {/* Mobile hamburger button — hidden when menu is open (rendered inside overlay instead) */}
        <button
          ...
          className={`relative lg:hidden z-[1001] flex h-[3.5em] w-[3.5em] items-center justify-center rounded-[0.75em] transition-colors duration-300 max-lg:[transition-property:color,background-color,transform,opacity] ${
            isMobileMenuOpen ? 'hidden' : 'flex bg-[var(--color-brand-pink)]'
          }`}
          style={{ boxShadow: 'none', outline: 'none', border: 'none', padding: 0 }}
    ...
    <AnimatePresence mode="wait">
      {isMobileMenuOpen && (
        <motion.div
          ...
          style={{ top: 0, left: 0, right: 0, bottom: 0, width: '100vw', height: '100svh' }}
          aria-modal="true"
          role="dialog"
          aria-label="Menu"
        >
          ...
          <div className="absolute top-0 left-0 right-0 z-[3] flex justify-between items-center w-full max-w-[96em] h-[5.6em] mx-auto px-[1em] md:px-[1.2em] lg:px-[1.6em]">
            <Link
              href="/"
              aria-label="Home link"
              className="relative h-[3em] inline-block"
              onClick={() => setIsMobileMenuOpen(false)}
            >
              {logoSvg}
            </Link>
            <button
              onClick={() => setIsMobileMenuOpen(false)}
              className="relative z-[1001] flex h-[3.5em] w-[3.5em] items-center justify-center rounded-[0.75em] bg-[var(--color-brand-white)] transition-colors duration-300"
              style={{ boxShadow: 'none', outline: 'none', border: 'none', padding: 0 }}
              aria-expanded={true}
          ...
          {/* === Content (nav links) on top of pink bg === */}
          <div className="relative z-[2] flex flex-col" style={{ width: '100%', height: '100svh' }}>
            {/* Spacer matching navbar height */}
            <div className="flex w-full shrink-0" style={{ height: '5.6em' }} aria-hidden="true" />

            {/* Centered nav links */}
            <div className="flex flex-1 flex-col items-center justify-center px-[1em]">
            ...
            {/* Bottom CTA button */}
            <div className="flex shrink-0 justify-center pb-8 pt-4">
              <motion.div
                initial={{ opacity: 0, y: '120%' }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: '120%' }}
                transition={{
                  delay: 0.15 + navLinks.length * 0.06 + 0.08,
                  duration: 0.68,
                  ease: [0.22, 1, 0.36, 1],
                }}
                style={{ transformOrigin: '50% 50%' }}
